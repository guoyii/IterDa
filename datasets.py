...
class BuildDataSet(Dataset):
    def __init__(self, data_root_path, folder, geo_full, geo_sparse, pre_trans_img=None, Dataset_name="test"):
        ...
        result_path_1 = self.root_path + "/results/Iter_1/v3/model/IterDa_E199_val_Best.pth"
        self.model_i1 =  torch.load(result_path_1, map_location=torch.device('cpu'))
        self.model_i1.eval()

        result_path_2 = self.root_path + "/results/Iter_2/v1/model/IterDa_E281_val_Best.pth"
        self.model_i2 =  torch.load(result_path_2, map_location=torch.device('cpu'))
        self.model_i2.eval()

        result_path_3 = self.root_path + "/results/Iter_3/v1/model/IterDa_E211_val_Best.pth"
        self.model_i3 =  torch.load(result_path_3, map_location=torch.device('cpu'))
        self.model_i3.eval()
    ...
    def __getitem__(self, idx):
        ...
        image_sparse_0 = self.fbp(sinogram_sparse, self.proj_id_sparse, self.proj_geom_sparse, self.vol_geom_sparse)
        ...
        image_sparse_1 = self.fbp(sinogram_new, self.proj_id_full, self.proj_geom_full, self.vol_geom_full)
        ...
        image_sparse_2 = self.fbp(sinogram_new, self.proj_id_full, self.proj_geom_full, self.vol_geom_full)
        ...
        image_sparse_3 = self.fbp(sinogram_new, self.proj_id_full, self.proj_geom_full, self.vol_geom_full)
